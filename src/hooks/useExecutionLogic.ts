...
export function useExecutionLogic(defaultCode: string) {
  ...
  // 🔍 Validación básica (opcional, mostly for UX hints)
  useEffect(() => {
    const handler = setTimeout(() => {
      const error = validateJavaSyntax(code);
  ...
  const consoleOutput = useMemo(() => {
    ...
    return executionTrace
      ...
      .filter((step): step is ExecutionStep => step !== undefined && step.consoleOutput !== undefined)
